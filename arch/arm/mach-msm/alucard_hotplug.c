			...
			unsigned long action, void *data)
{
	...
	case CPU_ONLINE:
		...
		pcpu_info->prev_cpu_idle = get_cpu_idle_time(cpu, &pcpu_info->prev_cpu_wall);
		pcpu_info->cur_up_rate = 1;
		pcpu_info->cur_down_rate = 1;
		break;
	}
...
static int hotplug_start(void)
{
	...
	alucard_wq = alloc_workqueue("alucard", WQ_HIGHPRI | WQ_FREEZABLE, 0);
	if (!alucard_wq) {
		pr_err("Alucard: Failed to allocate hotplug workqueue\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	return ret;
err:
	hotplug_tuners_ins.hotplug_enable = 0;
	return ret;
}
				   ...
				   const char *buf, size_t count)
{
	...
	if (hotplug_tuners_ins.hotplug_enable > 0) {
		for_each_online_cpu(j) {
			...
			pcpu_info->prev_cpu_idle = get_cpu_idle_time(j, &pcpu_info->prev_cpu_wall);
		}
	}
