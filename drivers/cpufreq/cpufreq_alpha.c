/*
 * drivers/cpufreq/cpufreq_alpha.c
 *
 * Copyright (C) 2011-2014 The XPerience PRoject
 * All of this work is from TeamMEX@XDA this is just a copy for alpha kernel MOTO E
 ...
 */
#define DEFAULT_RAMP_UP_STEP 998400
...
static DEFINE_PER_CPU(struct alpha_info_s, alpha_info);
...
static int cpufreq_governor_alpha(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ALPHA
static
#endif
struct cpufreq_governor cpufreq_gov_alpha = {
	.name = "alpha",
	.governor = cpufreq_governor_alpha,
	.max_transition_latency = 9000000,
	.owner = THIS_MODULE,
};

inline static void alpha_update_min_max(struct alpha_info_s *this_alpha, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_alpha->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_alpha->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void alpha_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct alpha_info_s *this_alpha = &per_cpu(alpha_info, i);
		if (this_alpha->enable)
			alpha_update_min_max(this_alpha,this_alpha->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct alpha_info_s *this_alpha) {
	this_alpha->time_in_idle = get_cpu_idle_time_us(cpu, &this_alpha->idle_exit_time);
	mod_timer(&this_alpha->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct alpha_info_s *this_alpha,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_alpha->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			printk(KERN_WARNING "alpha: frequency change failed: %d to %d => %d\n",
	...
	dprintk(ALPHA_DEBUG_JUMPS,"alphaQ: jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_alpha_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct alpha_info_s *this_alpha = &per_cpu(alpha_info, cpu);
	struct cpufreq_policy *policy = this_alpha->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_alpha->idle_exit_time == 0 || update_time == this_alpha->idle_exit_time)
		return;

	delta_idle = cputime64_sub(now_idle, this_alpha->time_in_idle);
	delta_time = cputime64_sub(update_time, this_alpha->idle_exit_time);

	// If timer ran less than 2ms after short-term sample started, retry.
	if (delta_time < 2000) {
		if (!timer_pending(&this_alpha->timer))
			reset_timer(cpu,this_alpha);
		return;
	}
	...
	dprintk(ALPHA_DEBUG_LOAD,"alphaT @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_alpha->cur_cpu_load = cpu_load;
	this_alpha->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_alpha->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_alpha->freq_change_time) >= up_rate_us))
		{
			dprintk(ALPHA_DEBUG_ALG,"alphaT @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_alpha->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_alpha->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_alpha->ideal_speed ||
		  cputime64_sub(update_time, this_alpha->freq_change_time) >= down_rate_us))
	{
		dprintk(ALPHA_DEBUG_ALG,"alphaT @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_alpha->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct alpha_info_s *this_alpha = &per_cpu(alpha_info, smp_processor_id());
	struct cpufreq_policy *policy = this_alpha->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_alpha->timer))
		del_timer(&this_alpha->timer);

	pm_idle_old();

	if (!timer_pending(&this_alpha->timer))
		reset_timer(smp_processor_id(), this_alpha);
}
...
static int cpufreq_idle_notifier(struct notifier_block *nb,
	...
	if (val == IDLE_START) {
		if (policy->cur == policy->max && !timer_pending(&this_alpha->timer)) {
			reset_timer(smp_processor_id(), this_alpha);
...
static void cpufreq_alpha_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	int ramp_dir;
	struct alpha_info_s *this_alpha;
	struct cpufreq_policy *policy;
	unsigned int relation = CPUFREQ_RELATION_L;
	for_each_possible_cpu(cpu) {
		this_alpha = &per_cpu(alpha_info, cpu);
		if (!work_cpumask_test_and_clear(cpu))
			continue;

		ramp_dir = this_alpha->ramp_dir;
		this_alpha->ramp_dir = 0;

		old_freq = this_alpha->old_freq;
		policy = this_alpha->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			printk(KERN_WARNING "alpha: frequency changed by 3rd party: %d to %d\n",
			       old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_alpha->ideal_speed)
				new_freq = this_alpha->ideal_speed;
			...
			dprintk(ALPHA_DEBUG_ALG,"alphaQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_alpha->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_alpha->ideal_speed) {
				...
				new_freq = old_freq * this_alpha->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(ALPHA_DEBUG_ALG,"alphaQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_alpha->ideal_speed);
		}
			...
			new_freq = old_freq;
			dprintk(ALPHA_DEBUG_ALG,"alphaQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_alpha,new_freq,old_freq,relation);
		if (new_freq)
			this_alpha->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_alpha->freq_change_time);
		...
		else if (timer_pending(&this_alpha->timer))
			del_timer(&this_alpha->timer);

		cpufreq_notify_utilization(policy,
			(this_alpha->cur_cpu_load * policy->cur) / policy->max);
	}
}
...
static struct attribute * alpha_attributes[] = {
...
static struct attribute_group alpha_attr_group = {
	.attrs = alpha_attributes,
	.name = "alpha",
};

static int cpufreq_governor_alpha(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_alpha->cur_policy = new_policy;

		this_alpha->enable = 1;

		alpha_update_min_max(this_alpha,new_policy,suspended);

		this_alpha->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_alpha->freq_table)
			printk(KERN_WARNING "alpha: no frequency table for cpu %d?!\n",cpu);
		...
		if (this_alpha->cur_policy->cur < new_policy->max && !timer_pending(&this_alpha->timer))
			reset_timer(cpu,this_alpha);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_alpha->cur_policy->cur > new_policy->max) {
			dprintk(ALPHA_DEBUG_JUMPS,"alphaI: jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_alpha->cur_policy,
						new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_alpha->cur_policy->cur < new_policy->min) {
			dprintk(ALPHA_DEBUG_JUMPS,"alphaI: jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_alpha->cur_policy,
...
static void alpha_suspend(int cpu, int suspend)
{
	struct alpha_info_s *this_alpha = &per_cpu(alpha_info, smp_processor_id());
	struct cpufreq_policy *policy = this_alpha->cur_policy;
	...
	alpha_update_min_max(this_alpha,policy,suspend);
	if (!suspend) { // resume at max speed:
		new_freq = validate_freq(policy,sleep_wakeup_freq);

		dprintk(ALPHA_DEBUG_JUMPS,"alphaS: awaking at %d\n",new_freq);
		...
		dprintk(ALPHA_DEBUG_JUMPS,"alphaS: suspending at %d\n",policy->cur);
	}

	reset_timer(smp_processor_id(),this_alpha);
}

static void alpha_early_suspend(struct power_suspend *handler) {
	int i;
...
static void alpha_late_resume(struct power_suspend *handler) {
	int i;
...
static struct power_suspend alpha_power_suspend = {
	.suspend = alpha_early_suspend,
	.resume = alpha_late_resume,
};

static int __init cpufreq_alpha_init(void)
{
	unsigned int i;
	struct alpha_info_s *this_alpha;
	debug_mask = 0;
	...
	for_each_possible_cpu(i) {
		this_alpha = &per_cpu(alpha_info, i);
		this_alpha->enable = 0;
		this_alpha->cur_policy = 0;
		this_alpha->ramp_dir = 0;
		this_alpha->time_in_idle = 0;
		this_alpha->idle_exit_time = 0;
		this_alpha->freq_change_time = 0;
		this_alpha->freq_change_time_in_idle = 0;
		this_alpha->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_alpha->timer);
		this_alpha->timer.function = cpufreq_alpha_timer;
		this_alpha->timer.data = i;
		work_cpumask_test_and_clear(i);
	}

	// Scale up is high priority
	up_wq = create_workqueue("kalpha_up");
	down_wq = create_workqueue("kalpha_down");
	if (!up_wq || !down_wq)
		return -ENOMEM;

	INIT_WORK(&freq_scale_work, cpufreq_alpha_freq_change_time_work);

	register_power_suspend(&alpha_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_alpha);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ALPHA
fs_initcall(cpufreq_alpha_init);
#else
module_init(cpufreq_alpha_init);
#endif

static void __exit cpufreq_alpha_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_alpha);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_alpha_exit);

MODULE_AUTHOR ("Carlos Jesús B.C (Klozz-TeamMEX)<xxx.reptar.rawrr.xxx@gmail.com>");
MODULE_DESCRIPTION ("'cpufreq_Alpha' - A smart cpufreq governor");
MODULE_LICENSE ("GPL");
